...
(function(){
    ...
    window.dataGridFieldFunctions = new Object()
    ...
    dataGridFieldFunctions.addRowOnChange = function(e,insert) {
        ...
        if(insert || (rows.length ==(tr.rowIndex))) {
            ...
            */
            ...
            jQuery('input', newtr).first().focus();
    ...
    registerXatafaceDecorator(function(root){
        $('table.xf-grid-table', root).each(function(){
            var thisTable = this;
            ...
            scripts = scriptTexts;
            $(this).attr('data-script-text', scripts.join("\n"));
            if ( typeof(dataGridFieldFunctions.templates) == 'undefined' ){
                dataGridFieldFunctions.templates = {};
            }

            dataGridFieldFunctions.templates['xf-grid-table-'+$(this).attr('data-grid-name')] = template.get(0);
            
            $('.xf-lookup-grid-row-button-'+fieldName, root).each(function(){
                ...
                if ( $(this).attr('data-filters') ){
                    config.filters = eval($(this).attr('data-filters'));
                }
                ...
                config.callback = function(values){
                    ...
                    var url = DATAFACE_SITE_HREF+'?-action=RecordBrowser_lookup_single&'+ids.join('&')+'&-table='+encodeURIComponent(tablename)+'&-text=__json__&-return-type=array';
