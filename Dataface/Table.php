...
class Dataface_Table
{
    ...
    function __construct($tablename, $db = null, $quiet = false)
    {
        ...
        if (
            !XF_USE_OPCACHE and DATAFACE_EXTENSION_LOADED_APC
            and
            (!@$_GET['--refresh-apc'])
            and
          ...
          */
            ...
            foreach ($fieldnames as $key) {
                $matches = array();
                if (
                    preg_match('/^(.*)_mimetype$/', $key, $matches) and
          ...
          ($this->isBlob($matches[1]) or $this->isContainer($matches[1]))*/
                    ...
                    preg_match('/^(.*)_filename$/', $key, $matches) and
                    isset($this->_fields[$matches[1]]) /*and
    ...
    function getDescriptionField()
    {
        if (!isset($this->descriptionField)) {
            ...
            $this->descriptionField = $this->guessField(
                array(
                    'text' => 10, 'mediumtext' => 10, 'shorttext' => 10, 'longtext' => 2,
                    'varchar' => 1, 'char' => 1
                ),
                array('/description|summary|overview/' => 10, '/desc/' => 2)
            );
        ...
        if (!isset($this->_cache[__FUNCTION__][intval($includeParent)])) {
            ...
            if (isset($del)) {
                ...
                foreach ($delegate_fields as $dfield) {
                    ...
                    if (
                        isset($this->_atts[$dfieldname]) and
    ...
    */
        ...
        foreach ($conf as $key => $value) {
            ...
            if (preg_match('/fieldgroup:(.+)/', $key, $matches)) {
                ...
                foreach ($grp as $grpkey => $grpval) {
                    ...
                    switch (count($tmp)) {
                        case 2:
                            $grp[$tmp[0]][$tmp[1]] = &$grp[$grpkey];
                            break;
                        case 3:
                            $grp[$tmp[0]][$tmp[1]][$tmp[2]] = &$grp[$grpkey];
                            break;
                        case 4:
                            $grp[$tmp[0]][$tmp[1]][$tmp[2]][$tmp[3]] = &$grp[$grpkey];
                            break;
                        case 5:
                            $grp[$tmp[0]][$tmp[1]][$tmp[2]][$tmp[3]][$tmp[4]] = &$grp[$grpkey];
                            break;
                    }
                }
                ...
                foreach ($value as $att => $attval) {
                    ...
                    if (count($attpath) == 1) {
                        ...
                        switch ($attpath[0]) {
                            ...
                            case "visibility":
                                if (!isset($field['visibility'])) $field['visibility'] = array(
                                    'list' => 'visible',
                                    'browse' => 'visible',
                                    'find' => 'visible'
                                );
    ...
    function parseImportData($data, $importFilter = null, $defaultValues = array())
    {
       ...
       */
            if (!isset($filters[$importFilter])) {
                return Dataface_Error::noImportFiltersFound(
                    "The import filter '" . $importFilter . "' was not found while attempting to import data into the table '" . $this->tablename . "'.  The following import filters are available: {" . implode(',', array_keys($errors)) . "}."
                );
            }
    ...
    {
        return Dataface_converters_date::time_to_string($value);
    }
